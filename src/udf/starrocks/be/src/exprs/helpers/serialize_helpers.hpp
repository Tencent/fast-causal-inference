...
public:
    template <typename T>
    static typename std::enable_if_t<std::is_arithmetic_v<std::remove_cv_t<std::remove_reference_t<T>>>, void>
    serialize(const T* data, uint8_t*& buffer, size_t num_elems = 1) {
    ...
    template <typename T>
    static typename std::enable_if_t<std::is_arithmetic_v<std::remove_cv_t<std::remove_reference_t<T>>>, void>
    deserialize(const uint8_t*& buffer, T* data, size_t num_elems = 1) {
    ...
    template <typename T>
    static typename std::enable_if_t<std::is_arithmetic_v<std::remove_cv_t<std::remove_reference_t<T>>>, void>
    serialize(const T& data, uint8_t*& buffer) {
        serialize(&data, buffer, 1);
    }

    template <typename T>
    static typename std::enable_if_t<std::is_arithmetic_v<std::remove_cv_t<std::remove_reference_t<T>>>, void>
    deserialize(const uint8_t*& buffer, T& data) {
    ...
    template <typename T>
    static typename std::enable_if_t<std::is_arithmetic_v<std::remove_cv_t<std::remove_reference_t<T>>>, size_t>
    serialized_size(T const&) {
        return sizeof(T);
    }
