...
Tree::Tree(UInt64 bucket_num_, UInt64 random_seed_, const ForestOptions& forest_options, const TreeOptions& tree_options) : bucket_num(bucket_num_), random_seed(random_seed_), // NOLINT
...
void Tree::predict(const ColumnsWithTypeAndName & arguments, UInt64 row_num, PaddedPODArray<Float64> & average_value) const
{
    ...
    for (size_t i = 1; i < arguments.size(); i++)
        line += arguments[i].name + ": " + std::to_string(arguments[i].column->getFloat64(row_num)) + ", ";
