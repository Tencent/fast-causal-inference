...
class TDWUtils(object):
    ...
    NAME_SPACE = PROJECT_CONF["tdw"]["hdfs_namespace"]
    BASE_HDFS_PATH = PROJECT_CONF["tdw"]["hdfs_base_path"]
    MAX_FILE_COUNT = 3600
    MAX_LENGTH = 50  # GB
    ...
    def tdw_2_clickhouse_distribute(self, tdw_database_name, tdw_table_name, clickhouse_table_name, spark, cmk=None,
        ...
        for i in range(1000):
            if i < 10:
                hdfs_path_list[i % clickhouse_utils.cluster_hosts_len].append("00" + str(i))
            elif i < 100:
                hdfs_path_list[i % clickhouse_utils.cluster_hosts_len].append("0" + str(i))
            else:
        ...
        def clickhouse_node_job(clickhouse_utils, clickhouse_table_name, external_table_name, colname_statement,
            ...
            logger.debug(str(clickhouse_utils.host) + ", location=" + str(location))
            ...
            logger.debug(str(clickhouse_utils.host) + ", external_table_name=" + external_table_name + ", location=" +
                         str(location) + ",tdw rows = " + str(clickhouse_utils.table_rows(external_table_name)))
            if sql:
                ...
                clickhouse_utils.insert_table(clickhouse_table_name, external_table_name, colname_statement,
                                              col_if_statement)
            logger.debug(str(clickhouse_utils.host) + ", insert done, table_name=" + clickhouse_table_name + ", location="
                         + str(location) + ",tdw rows = " + str(clickhouse_utils.table_rows(clickhouse_table_name)))
            clickhouse_utils.close()
