...
#from fast_causal_inference.databus.clickhouse import *
...
class LongTerm:
    ...
    def call_func(self, sql, sample_num):
        bs_param = self.sql_instance.sql("select DistributedNodeRowNumber() from " + self.exe_table).values[0][0]
        bs_param_str = ''
        for x in bs_param:
            for y in x:
                bs_param_str += y
        ...
        self.forward_sql = self.forward_sql.replace('sample_num =', '').replace('bs_num =', '').replace('bs_param = ', '')
    ...
    def __str__(self):
        print('mean0       mean1       estimate    stderr      t-statistic p-value     lower       upper')
        return str(self.res)
...
def ttest(result):
    ...
    std = np.sqrt(result[1,2]/result[1,3]+result[0,2]/result[0,3]) # * np.sqrt(3507712.0/95885423.0)
...
def longterm_plot(df):
    ...
    axs.plot(ground_truth_['t'], ground_truth_['mean'], '--', color='#636e72', label = 'true effect')
    axs.fill_between(ground_truth_['t'],ground_truth_['lower'] , ground_truth_['upper'],alpha = 0.2, color='#636e72')
    axs.plot(effect['t'], effect['mean'], '-o', markersize=2, color='#e74c3c', label = 'estimation')
    axs.fill_between(effect['t'], effect['lower'] , effect['upper'],alpha = 0.2, color='#e74c3c')
...
def Long_term_result(table,surrogates,T,
                     train_Ts='2~1',predict_Ts='3-5',
                     key_metric=1,treatment='groupid',
                     model='Ols(False)',bs_num=100):   
    x_list_string = "[" + ", ".join(["(" + f"{i}, ".join(surrogates) + f"{i})" for i in range(1,T+1)]) + "]"
    ...
    x = LongTerm(string,
             table=table,sample_num=cnt, bs_num = bs_num, cluster = 'mmdcchfishertest')
    ...
    pred_df = pd.DataFrame(result,columns=['mean_pred','std_pred','lower_pred','upper_pred'])
