from .. import create_sql_instance, clickhouse_create_view, clickhouse_drop_view, clickhouse_2_dataframe, \
...
def ROC_curve(table, label, P):
    ...
    thresholds_str = sql_instance.sql(f"select quantiles({','.join(quantiles)})({P}) as res from {table}").values[0][
    ...
    order by threshold
    """
    results = sql_instance.sql(query)[['threshold', 'TP', 'FP', 'TN', 'FN']].astype(float).values
    roc_data = []
...
def LogisticRegression_spark(spark, table, label, X, categorical_columns, maxIter=40, regParam=0, elasticNetParam=0.8):
    ...
    encoders = [OneHotEncoder(inputCol=column + "_index", outputCol=column + "_onehot") for column in
                categorical_columns]
    ...
    # 使用withColumn添加新列
    predictions = predictions.withColumn("score", get_first_element_udf(predictions["probability"]))

    predictions = predictions[data.columns + ['score']]
    predictions = predictions.drop('day_').drop('id')
    table_output = f'{table}_{int(time.time())}'
    dataframe_2_clickhouse(dataframe=predictions, clickhouse_table_name=table_output, is_auto_create=True)
    print('*' * 30)
